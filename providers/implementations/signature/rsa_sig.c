...
static int wrap_md_dup(PROV_RSA_CTX *dstctx, PROV_RSA_CTX *srcctx)
{
    if (srcctx->md.md != NULL) {
        ...
        if (srcctx->mdctx != NULL)
            if ((dstctx->mdctx = EVP_MD_CTX_new()) == NULL
...
static int rsa_setup_md(PROV_RSA_CTX *ctx, const char *mdname,
                        const char *mdprops, const OSSL_DISPATCH *mddispatch,
                        int mdnid, const char *desc)
{
    EVP_MD *md = NULL;
    ...
    if (mdname != NULL) {
        ...
        if (!ctx->flag_allow_md) {
            if (ctx->md.md != NULL && !wrap_md_is_a(ctx, mdname, md_nid)) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_DIGEST_NOT_ALLOWED,
